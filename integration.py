...
def run_classical_pipeline(frame):
    ...
    if np.mean(cv2.cvtColor(tiny, cv2.COLOR_BGR2GRAY)) > 180:
        img = cv2.convertScaleAbs(img, alpha=1.3, beta=-80)

    roi_mask_img = np.zeros((height, width), dtype=np.uint8)
    cv2.rectangle(roi_mask_img, (int(width * 0.25), int(height * 0.35)), (int(width * 0.70), int(height * 0.70)), 255, -1)
    ...
    if lines is not None:
        lines = lines[:, 0, :]
        angles = np.abs(np.arctan2(lines[:, 3] - lines[:, 1], lines[:, 2] - lines[:, 0]) * 180.0 / np.pi)
...
def run_lane_pipeline(frame):
    ...
    edges = cv2.Canny(cv2.GaussianBlur(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), (9, 9), 0), 70, 100)
    
    # ROI setup
    mask = np.zeros_like(edges)
    pts_roi = np.array([[(int(width * 0.05), height), (int(width * 0.95), height), 
                         (int(width * 0.65), int(height * 0.40)), (int(width * 0.35), int(height * 0.40))]], dtype=np.int32)
    ...
    y_left, y_right = y_left + int(height * 0.5), y_right + int(height * 0.5)

    # REFINED HYBRID FILTER: Slope Rejection + Tighter Statistical Cleaning
    def filter_points(x, y, midpoint, height):
        if len(x) == 0: return np.array([]), np.array([])
        
        # 1. Slope: Reject horizontal paving/kerb edges
        slope_mask = np.array([abs(yi - height) / (abs(xi - midpoint) + 1e-5) > 0.4 for xi, yi in zip(x, y)])
        x, y = x[slope_mask], y[slope_mask]
        
        if len(x) == 0: return np.array([]), np.array([])
        
        # 2. Statistical: Remove X outliers from the kerb/paving subset
    ...
    if len(x_left) > 50:
        ...
        cv2.polylines(line_image, [np.array([np.transpose(np.vstack([left_func(np.linspace(height//2, height-1, 50)), np.linspace(height//2, height-1, 50)]))], np.int32)], False, (0, 255, 0), 6)
    ...
    if len(x_right) > 50:
        ...
        cv2.polylines(line_image, [np.array([np.transpose(np.vstack([right_func(np.linspace(height//2, height-1, 50)) + midpoint, np.linspace(height//2, height-1, 50)]))], np.int32)], False, (0, 255, 0), 6)

    lane_center_offset = (left_x_bottom + right_x_bottom) // 2 - midpoint if (left_x_bottom is not None and right_x_bottom is not None) else None
...
def make_decision(barrier_detected, yolo_detections, lane_center_offset):
    if barrier_detected: return "STOP - BARRIER", (0, 0, 255)
    if any(d['distance'] == 'CLOSE' for d in yolo_detections): return "STOP - OBSTACLE", (0, 0, 255)
    if lane_center_offset is not None:
        if lane_center_offset > LANE_OFFSET_THRESHOLD: return "TURN RIGHT", (0, 165, 255)
        if lane_center_offset < -LANE_OFFSET_THRESHOLD: return "TURN LEFT", (0, 165, 255)
...
def process_video(video_path, filename):
    ...
    out = cv2.VideoWriter(os.path.join(OUTPUT_FOLDER, f"processed_{filename}"), cv2.VideoWriter_fourcc(*'mp4v'), 30, (1280, 720))
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        while cap.isOpened():
            ...
            f_bar = executor.submit(run_classical_pipeline, frame)
            ...
            for det in yolo_dets:
                ...
                cv2.putText(output_frame, det['label'], (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, det['color'], 2)

            if bar_det and bar_line is not None:
                cv2.line(output_frame, (bar_line[0], bar_line[1]), (bar_line[2], bar_line[3]), (255, 0, 0), 4)

            decision, d_color = make_decision(bar_det, yolo_dets, offset)
            output_frame = draw_overlay(output_frame, decision, d_color, fps_display, yolo_dets, offset)
...
if __name__ == "__main__":
    ...
    for vid in v_files: process_video(os.path.join(VIDEO_FOLDER, vid), vid)
